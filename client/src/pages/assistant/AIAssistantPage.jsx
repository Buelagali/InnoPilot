  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {activeProject && (
          <Badge variant="primary" className="text-xs px-3 py-1.5">
        ...
        {shortcutCards.map((sc, i) => {
          ...
          return (
            <GlassCard
              ...
              className="p-3.5 cursor-pointer flex flex-col justify-between group"
            >
              <div className="flex items-center gap-2.5 mb-2">
                <div className="w-7 h-7 rounded-lg bg-indigo-600/20 text-indigo-400 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
              ...
              <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed">{sc.prompt}</p>
            </GlassCard>
          );
        })}
      </div>
      ...
      <GlassCard className="p-5 sm:p-6 border-indigo-500/30 flex flex-col h-[580px]">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          {messages.map((m, idx) => (
            ...
            >
              {m.sender === 'ai' && (
                <div className="w-8 h-8 rounded-xl bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center text-indigo-300 flex-shrink-0 mt-0.5">
          ...
          {loading && (
            <div className="flex items-center gap-2.5 text-indigo-400 text-xs italic pl-2">
        ...
        {/* Input Bar */}
        <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="mt-4 pt-3 border-t border-white/10 flex gap-2">
          <input
            type="text"
            value={inputMessage}
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs sm:text-sm font-semibold disabled:opacity-50 transition-colors flex items-center gap-1.5"
          >
